            ...
            <div className='flex-1'>
                <div className='flex justify-between items-center'>
                    <div>
                        ...
                        <small className="ml-2 text-sm text-gray">{dayjs(post.created_at).fromNow()}</small>
                        {post.created_at !== post.updated_at && <small className='text-sm text-gray-600'>&middot; edited</small>}
                    </div>
                    {/* Si el usuario logueado es la misma persona que creo el post. */}
                    {post.user.id === auth.user.id &&
                        <Dropdown>
                            <Dropdown.Trigger>
                                {/* Mostramos el dropdwn para editar o eliminar. */}
                                <button>
                                    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-gray" viewBox="0 0 20 20" fill="currentColor">
                ...
                {editing
                    ...
                    : (
                        <>
                            <p className="mt-4 text-lg text-gray">{post.title}</p>
                            <p className="mt-4 text-gray">{post.body}</p>
                        </>
                    )
                }
